...
- **Abort mechanic**: killing the in-flight child (`child.kill()`) is straightforward, but needs to happen cleanly mid-stream (partial stdout parsing, no corrupted `--resume` session state for the retry).
