...
- Xcode: **26.0.1 (17A400) with the iOS 26.0 SDK**, not Xcode 27 *(2026-09-16: Mac upgraded to macOS 27.0 and Xcode 27.0 / iOS 27 SDK; Chris's iPhone is on iOS 27)*. The POC builds against iOS 26 (`IntentModes`/`supportedModes` are iOS 26 APIs, so nothing in the POC needs 27). Installing to an iOS 27 device needs Xcode 27's device support. No code-signing identity is present on the Mac yet (`security find-identity` → 0), so the first device install is done from Xcode after signing in with an Apple ID.
...
Checked 2026-09-15 against the **iOS 26.0 SDK** shipped with Xcode 26.0.1, then re-checked 2026-09-16 after the Mac moved to macOS 27 + **Xcode 27.0 (27A266a) / iOS 27.0 SDK**. The app's deployment target is now 27.0.
...
| `AppIntentsTesting` invocation API | per WWDC26-295 | Not in Xcode 26. **Present in Xcode 27** (`Platforms/iPhoneOS.platform/Developer/Library/Frameworks/AppIntentsTesting.framework`, iOS 27+): `IntentDefinitions(bundleIdentifier:)` → `.intents["AskPeggyIntent"].makeIntent(question: …)` → `AnyAppIntent.run() async throws -> ResolvedIntentResult`. `ResolvedIntentResult` is `@dynamicMemberLookup` over `value` only — **there is no dialog accessor** | Partly — `PeggyTests/AskPeggyIntentFrameworkTests.swift` builds the intent through the framework and calls `run()`, but on the Xcode 27.0 / iOS 27.0 simulator driven by `xcodebuild test` the call fails with `transportCancelled` before `perform()` runs (same with ad-hoc signing on, and after launching the app once so its `Metadata.appintents` is registered; the simulator log only shows `siriactionsd`'s XPC connection to `linkd` being invalidated). The test skips on that error so the failure stays visible. The FR-20 dialog strings are asserted in `AskPeggyIntentTests` via `AskPeggyIntent.run(question:waitBudgetSeconds:)` → `AskPeggyOutcome.dialogText`; the framework could not read dialog anyway. Revisit in E45 S45.4 under Xcode's own runner / on device |
| `LongRunningIntent.performBackgroundTask` | per docs | Not in the iOS 26 SDK; **present in the iOS 27 SDK** (`protocol LongRunningIntent`) | Not needed for E44; available for E45's hand-off spike |
