...
E9 closes the memory loop by injecting historical context into new sessions and enabling Peggy to actively recall relevant memories. Where E8 extracts and stores knowledge from past conversations, E9 surfaces that knowledge at the moment it's needed — delivering session summaries when a new Claude Code session starts and providing FTS5-powered recall tools so Peggy can query her memory on demand. After E9, Peggy has genuine continuity: she knows what she's discussed before and can pick up where she left off.
...
- When a new Claude Code session starts, Peggy automatically receives a `session_history` event containing summaries from the past 48 hours
...
**User story:** As Peggy, I want to automatically receive a session history briefing when I start a new conversation so that I have relevant context about recent interactions without having to manually ask for it.
...
**User story:** As Peggy, I want the `recall_memory` tool to perform ranked full-text search across my memory store so that I can quickly surface relevant facts about a contact or topic without reading through raw transcripts.
...
- **`session_history` event timing:** The ContextInjector fires the event during adapter startup. This means Peggy sees the history before her first user message arrives. This is the desired UX — she should already be oriented when Chris says "hello." Make sure the CC adapter awaits the inject call before signaling readiness.
...
- **`/resume` vs. `get_briefing`:** These are related but different. `get_briefing` (E7) returns summary data for the *current* session (or a specified one). `/resume` is a command that actively fires a past session's context into the conversation stream. The difference is push vs. pull.
