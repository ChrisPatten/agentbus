...
- **Stage 70 — Route-Resolve:** Looks up `context.envelope.recipient_id` against routing rules in `AppConfig.pipeline.routes[]`; resolves to a list of `{ adapterId, recipientId }` tuples; sets `context.routes[]`; if no rule matches, defaults to `claude` on `claude-code` channel
...
- **Route-resolve fallback:** Defaulting to `claude@claude-code` when no rule matches means unrouted messages always reach the agent. This is the right behavior for a agent bus. Add a config flag `pipeline.dropUnrouted: true` for cases where this isn't desired.
