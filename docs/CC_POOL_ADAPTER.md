...
- Two messages for the same conversation arriving while a relaunch is in progress can't trigger two relaunches or get routed to a different pane: the relaunch first atomically claims the lease row (`leased` -> `launching`, scoped to that exact conversation), the same state a fresh claim already uses — a second concurrent call either loses that claim, or, if it arrives before the claim, matches the row as still-`launching` for this conversation and is treated as a `reuse` rather than being sent to a different free pane. Either way, that second call does **not** return immediately: it *waits* for the first call's launch or relaunch to actually finish (tracked per pane, cleared once the pane is confirmed ready or marked dead — never once the mere network call returns) before resolving. This matters because the caller enqueues the inbound message right after `resolveRoute()` returns — if a losing call returned the pane's id immediately, the message could be enqueued while the OLD Claude session (about to be killed by the winning call's `release(..., 'kill')`) is still alive and polling, which could ack it into a session that's a moment away from dying, losing it. Once the wait ends, the row is re-read fresh: a pane that came up `dead` (the relaunch failed) or moved to a different conversation parks the message instead of addressing it to a pane that isn't ready or isn't this conversation's anymore. The same wait-then-reread also covers a brand-new conversation's very first message racing its own initial `bound`/`grow`/`evict` launch, not just a model-switch relaunch.
