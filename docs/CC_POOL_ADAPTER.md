...
| `leased` → `draining` | The pane's `cc.ts` actually polled a message off `GET /api/v1/messages/pending` — `LeaseStore.beginTurn()`, called from that same handler. Marks the pane as mid-turn, not idle. |
| `draining` → `leased` | The pane replied — `POST /api/v1/messages` calls `LeaseStore.endTurn()` for the sending pane's agent id once its outbound send has passed the [stale-sender guard](#allocation-and-eviction) and been enqueued. Also resets the idle clock: `last_activity_at` is stamped at the moment the turn actually ended, not when it began. |
| `leased` → `free` | The lease is released: the hard-idle sweep found it past `lease.hard_idle_ms`, or a new conversation's `acquire()` evicted it (least-recently-active, past `lease.idle_evict_ms` — `draining` panes are never eviction-eligible this way, see below). Either way, `on_evict` runs against the pane. |
| `draining` → `free` | The hard-idle sweep found it past `lease.hard_idle_ms`, same as a stale `leased` pane — the backstop for a turn that never actually replies (so `endTurn()` never runs). `acquire()`'s eviction path does *not* do this: a `draining` pane is never picked for eviction, however idle it looks by timestamp alone. |
...
`draining` marks a pane whose in-flight turn shouldn't be reused until that turn completes. `LeaseStore.acquire()`'s LRU eviction (see [Allocation and eviction](#allocation-and-eviction)) only ever selects `state = 'leased'` rows, so a pane sitting in `draining` — however long its turn takes — can never be reassigned out from under it. This closes a real gap: without it, a pane handling a single-shot delivery (a scheduled trigger with no follow-up inbound message, e.g. a morning brief) never got `last_activity_at` touched again after the initial claim, so a slow turn — fetching data, composing a reply — could look idle to a completely different conversation's `acquire()` call and get evicted before it ever replied. The evicted pane's eventual `reply()`/`send_message()` call then hit the [outbound lease guard](#allocation-and-eviction) and was rejected with a `409` and dropped, with no retry — the guard is a last-resort backstop for exactly this race, not something normal operation should rely on triggering. `reconcileLiveness()` still treats `draining` the same as `leased` for liveness-checking (a crashed pane is a crashed pane regardless of what it was doing), and the hard-idle sweep still reclaims a stuck `draining` pane past `lease.hard_idle_ms` — so a turn that never calls back out at all doesn't permanently strand a pane, it just costs the pool one pane for up to `lease.hard_idle_ms`, same as before.
...
|---|---|---|
| `reuse` | This conversation already holds a `leased` **or `draining`** pane. | The lease is touched (activity timestamp bumped); no launch needed. |
...
Eviction always picks the least-recently-active idle **`leased`** pane, never a busy one — a `draining` pane (an in-flight turn, per [Pane lifecycle](#pane-lifecycle) above) is never eviction-eligible no matter how long its turn has been running, and a `leased` pane with recent activity is not eviction-eligible either no matter how long its conversation has existed. Reuse is checked against `leased` **and** `draining` together for the same reason as eviction is checked against `leased` alone: a follow-up message for a conversation that's still mid-turn must find its own pane, not fail the lookup and get bound onto a second one.
...
**Hard-idle release** runs independently of new demand: on every sweep tick (60 seconds — a fixed interval, not currently configurable), any `leased` **or `draining`** pane idle past `lease.hard_idle_ms` is released, `on_evict` applied, even when no other conversation is waiting for it — so a pool doesn't sit fully leased forever on conversations that trailed off, and a turn that never actually replies (so never transitions back out of `draining`) doesn't strand a pane forever either.
...
**Outbound lease guard.** Even with `draining` now protecting a pane through however long its own turn takes, a slow reply from an *already-superseded* turn — one whose pane went on to `hard_idle_ms`-expire, get reassigned, and start a second turn — could still land after the pane has moved on to a new conversation. `POST /api/v1/messages` guards against this: it resolves the outbound message's target conversation, looks up the sending pane's current lease by its agent id, and rejects with `409` if that pane is `leased` **or `draining`** for a different conversation than the one the reply targets — `draining` is included because a pane that's busy again, on someone else's turn, is exactly as stale a sender as one that's merely idle and reassigned. This is enforced for every outbound send a pane's agent id can trigger — `reply`, `send_message`, and `send_email` all thread the pane's real `AGENTBUS_AGENT_ID` through as the sender for exactly this check. A send that clears the guard also ends the pane's own turn (`draining` → `leased`, with `last_activity_at` reset to now) via the same call.
...
| bus-core logs `[pool-guard] stale sender: pane agent:<pool>-pool-<n> is no longer leased to this conversation`, and the intended recipient never gets the reply | Historical bug, fixed: a pane's `last_activity_at` was only ever bumped at claim time and on a follow-up inbound message (`acquire()`'s `reuse` branch) — nothing touched it while the pane was actually composing a reply. A single-shot delivery with no follow-up message (a scheduled trigger, e.g. a morning brief) froze the timestamp the moment work started; if that turn ran long enough relative to `lease.idle_evict_ms`, a *different* conversation's `acquire()` call saw an apparently-idle pane and evicted it mid-turn, and the original turn's eventual reply then hit this guard and was dropped with no retry. Fixed by wiring up the pane lifecycle's `draining` state (see [Pane lifecycle](#pane-lifecycle)): a pane is marked `draining` the moment it's actually handed a message and can't be evicted by LRU while `draining`, regardless of how long the turn takes. If you still see this on a current build, it points at a genuinely superseded turn (the pane's prior conversation was reassigned via hard-idle release — `lease.hard_idle_ms` — while that turn never called back out at all) rather than this original race; check `/pool` for how long the pane has actually been on its current conversation. |
