...
- Two messages for the same conversation arriving while a relaunch is in progress can't trigger two relaunches or get routed to a different pane: the relaunch first atomically claims the lease row (`leased` -> `launching`, scoped to that exact conversation), the same state a fresh claim already uses — a second concurrent call either loses that claim, or, if it arrives before the claim, matches the row as still-`launching` for this conversation and is treated as a `reuse` rather than being sent to a different free pane. Either way, that second call does **not** return immediately: it *waits* for the first call's release-and-launch (or release-and-relaunch) to actually finish before resolving. Tracking starts the instant the pane is claimed — before its `release()` call runs, not just around the `launch()` call after it — because the danger window is the release itself: the pane's prior occupant (the OLD Claude session being relaunched, or, on an `evict` claim, the displaced conversation's session) is still alive and polling for the entire duration of `release()`, including under `on_evict: clear`, which never kills the process at all. If a losing call returned the pane's id immediately during that window, the caller's very next step — enqueuing the inbound message — could hand it to a session that's either about to be killed (relaunch, or `on_evict: kill`) or about to be reassigned to a different conversation (`on_evict: clear`), acking and losing it either way. Once the wait ends, the row is re-read fresh: a pane that came up `dead` (the launch failed) or moved to a different conversation parks the message instead of addressing it to a pane that isn't ready or isn't this conversation's anymore. The same wait-then-reread also covers a brand-new conversation's very first message racing its own initial `bound`/`grow`/`evict` claim, not just a model-switch relaunch.
