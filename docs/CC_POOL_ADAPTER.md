...
| `launch_ack_delay_ms` | number (ms) | `500` | Delay before sending `Enter` to acknowledge the experimental MCP-channel launch prompt. |
| `launch_ack_max_attempts` | number | `3` | Maximum re-sends of `Enter` if the pane still shows the ack prompt. |
| `launch_ack_pattern` | string | `"experimental"` | Text matched in pane output to detect the ack prompt. Override if a CLI update rewords it. |
...
4. **Acknowledge the experimental-channel prompt.** A freshly started session — fresh or resumed — blocks on a confirmation prompt for the channel-notification feature. After `launch_ack_delay_ms`, the pane sends `Enter` and checks `capture-pane` output for `launch_ack_pattern`; if the prompt is still showing, it retries on a short backoff, up to `launch_ack_max_attempts` times, before giving up.
5. **Confirm readiness.** The pane isn't handed a message until its `cc.ts` has actually polled the bus: `PoolManager` checks `GET /api/v1/agents/<pane agent id>/last-poll` until it reports a poll at or after the launch started, or a 30-second launch timeout elapses.
